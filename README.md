...
My aesthetic lodestar is a "film look" in the broad sense, but rather than emulating any specific stock or process, I’m more chasing my personal "sense memories" of projected film. However, these tools are (hopefully) flexible enough to achieve any number of looks/styles.
...
`PrimeraHue.dctl` performs per-channel hue and density control via tetrahedral interpolation, based on [hotgluebanjo](https://github.com/hotgluebanjo/TetraInterp-DCTL)’s DCTL implementation of the approach described by Steve Yedlin ([DisplayPrep](https://www.yedlin.net/DisplayPrepDemo/index.html), 2018).
...
`PrimeraPlus.dctl` is Primera's full control set plus PrimeraHue's tetrahedral hue/density interpolation in a single DCTL. Intended for look development (Group/Timeline nodes) or when hue shifts are needed alongside primary corrections at the clip level.

### Primera Skin

`PrimeraSkin.dctl` is a dedicated skintone correction tool operating in the OKLCH color model. Targets a soft region of the HSV disc centered on the nominal skin tone hue (~28°) and applies perceptually uniform adjustments only within that region. Everything outside the mask passes through untouched.
...
`PrimeraSplit.dctl` provides subtractive color split-toning for imbuing shadows and highlights with contrasting color casts. Can be thought of less as an "effect” and more a fundamental look development tool, defining the chromatic side of the overall characteristic curve.

- **Pivot** — Defaults to the selected transfer function's mid-grey (but should be set purely by eye)
...
- Maybe/hopefully it goes without saying at this point, but the Primera tools are meant to be used in the context of a fully color-managed workflow.
- The Primera tools should play nicely with most DRTs (Display Rendering Transform, the final color managment pipeline stage before outputting a deliverable) but has been used/tested the most with [Jed Smith](https://github.com/jedypod)'s [OpenDRT](https://github.com/jedypod/open-display-transform)
- I also regularly use/test the tools with Resolve's CST, [Juan-Pablo Zambrano](https://github.com/JuanPabloZambrano)'s excellent [2499 DRT](https://github.com/JuanPabloZambrano/DCTL/tree/main/2499_DRT), the [ACES 2.0](https://acescentral.com) [transforms](https://github.com/aces-aswf/aces-core), and occasionally LUTs
